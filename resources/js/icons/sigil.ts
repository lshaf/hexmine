 ...
 * everybody has to recognise. So it gets a mark: seven flat-top hexes in a
 ...
 * centre cell always burns, so every seal reads as one object rather than a
 ...
 *
 * The generated markup contains only palette colours and numbers. No part of the
...
/** Dark cells are the plate showing through, not a colour of their own. */
const DARK = '#2c3730'
...
/** Centre, then the six neighbours of a flat-top hex, clockwise from the top. */
...
export function walletSeal(wallet: string, size = 34): string {
  ...
  const cells = CELLS.map(([dx, dy], i) => {
    // The centre always burns; the ring carries the entropy.
    const state = i === 0 ? 1 : randInt(hash2(seed, i, 0x53a2), 0, 2)
    const fill = state === 0 ? DARK : state === 1 ? accent : VELLUM_DIM
    return `<path d="${hexPath(CENTRE + dx, CENTRE + dy, CELL)}" fill="${fill}"/>`
  }).join('')
